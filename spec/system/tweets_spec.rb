...
RSpec.describe "ツイート編集", type: :system do
  ...
  context "ツイート編集ができるとき" do
    it "ログインしたユーザーは自分が投稿したツイートの編集ができる" do
      ...
      # 投稿内容を編集する
      fill_in "tweet_image", with: "#{@tweet1.image}+画像を編集しました"
      fill_in "tweet_text", with: "#{@tweet1.text}+テキストを編集しました"
      ...
      expect(page).to have_selector ".content_post[style='background-image: url(#{@tweet1.image}+画像を編集しました);']"
      # トップページには先ほど変更した内容のツイートが存在する（テキスト）
      expect(page).to have_content("#{@tweet1.text}+テキストを編集しました")
    end
  end
