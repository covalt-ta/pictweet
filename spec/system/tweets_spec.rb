...
RSpec.describe 'ツイート編集', type: :system do
  ...
  context 'ツイート編集ができるとき' do
    it 'ログインしたユーザーは自分の投稿の編集ができる' do
      ...
      # 投稿内容を編集する
      fill_in "tweet_image", with: "#{@tweet1.image}を編集しました"
      fill_in "tweet_text", with: "#{@tweet1.text}を編集しました"
      ...
      expect(page).to have_selector ".content_post[style='background-image: url(#{@tweet1.image}を編集しました);']"
      # トップページには先ほど変更した内容のツイートが存在する（テキスト）
      expect(page).to have_content("#{@tweet1.text}を編集しました")
    end
  end
...
RSpec.describe 'ツイート詳細', type: :system do
  ...
  it 'ログインしていない状態でツイート詳細ページに遷移できるがコメント投稿欄が表示されない' do
    # トップページに移動する
    visit root_path
    # ツイートに「詳細」ボタンがある
    expect(
